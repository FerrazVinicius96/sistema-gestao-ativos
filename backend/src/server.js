...
app.get('/api/reports/tablets/consolidated/pdf', authenticateToken, authorizePermission('MENU_ESCOLAR'), async (req, res) => {
    try {
        ...
        const logoPath = path.join(__dirname, '../assets/institution-logo.png');
        let logoBase64 = null;
    ...
    const docDefinition = {
      ...
      footer: {
        ...
        stack: [
            ...
            { text: 'Endereço da unidade administrativa', alignment: 'center', fontSize: 9 },
            { text: 'CEP: 00000-000 | FONE: (00) 0000-0000', alignment: 'center', fontSize: 9 }
        ]
      },
      ...
      content: [
        ...
        // >>> ITEM 1 ATUALIZADO COM A NOVA ORIENTAÇÃO <<<
        {
            text: [
                ...
                { text: 'contato-ti@orgao.exemplo.gov.br', bold: true, decoration: 'underline', color: 'blue' },
        ...
        },
        {
            text: [
                { text: '5.2. ', bold: true },
                'Enviar um ofício relatando o ocorrido, com a cópia anexa do Boletim de Ocorrência, para a GIT – Gerência de Infraestrutura de Tecnologia, localizada à Endereço da unidade administrativa, Recife (nome do prédio), por e-mail (contato@orgao.exemplo.gov.br e infraestrutura@orgao.exemplo.gov.br).'
            ],
            style: 'body',
    ...
    const docDefinition = {
      ...
      footer: {
        ...
        stack: [
            ...
            { text: 'Endereço da unidade administrativa | CEP: 00000-000 | FONE: (00) 0000-0000', alignment: 'center', fontSize: 8 }
        ]
      },
...
app.post('/api/reports/blank-collection-order', authenticateToken, async (req, res) => {
    try {
        ...
        // 1. Logo
        const logoPath = path.join(__dirname, 'assets/institution-logo.png'); // Ajuste se necessário '../assets'
        let logoBase64 = null;
        if (fs.existsSync(logoPath)) {
...
app.post('/api/reports/preview-return-term', authenticateToken, async (req, res) => {
  try {
    ...
    let logoBase64 = null;
    const pathsToTry = [
        path.join(__dirname, 'assets/institution-logo.png'),
        path.join(__dirname, '../assets/institution-logo.png'),
        path.join(__dirname, 'public/assets/institution-logo.png')
    ];
    ...
    const docDefinition = {
      ...
      header: getHeader(),
      footer: (page) => ({ margin: [40, 0, 40, 10], stack: [ { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 1 }] }, { text: 'Endereço da unidade administrativa | CEP: 00000-000 | FONE: (00) 0000-0000', alignment: 'center', fontSize: 7, margin: [0, 3, 0, 0] } ] }),
...
app.post('/api/reports/preview-substitution', authenticateToken, async (req, res) => {
  try {
    ...
    // 1. Configurar Logo
    const logoPath = path.join(__dirname, 'assets/institution-logo.png'); // Confirme se é 'assets' ou '../assets' no seu projeto
    let logoBase64 = null;
    if (fs.existsSync(logoPath)) {
        try { logoBase64 = fs.readFileSync(logoPath, 'base64'); } catch(e) {}
    }
    ...
    const docDefinition = {
      ...
      footer: {
        stack: [
          ...
          { text: 'Endereço da unidade administrativa | FONE: (00) 0000-0000', alignment: 'center', fontSize: 8 },
        ],
        style: 'footer'
      },
...
app.get('/api/asset-movements/:id/receipt-pdf', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (movement.request_channel_type === 'Email') requestSource = 'E-mail para contato@orgao.exemplo.gov.br';
    ...
    if (isReturn) {
        ...
        termBodyText = [
             ...
             { text: ' 4.2 Enviar um ofício citando o ocorrido com cópia anexa do Boletim de Ocorrência para a Gerência de Tecnologia, localizada à Endereço da unidade administrativa, Recife (nome do prédio) por e-mail (contato@orgao.exemplo.gov.br e infraestrutura@orgao.exemplo.gov.br).', style: 'legalText', margin: [10, 0, 0, 15] }
        ];
    }
    ...
    const getFooter = () => ({
        margin: [30, 0, 30, 10], stack: [{ canvas: [{ type: 'line', x1: 0, y1: 0, x2: 535, y2: 0, lineWidth: 1 }] }, { text: 'Gerência de Infraestrutura de Tecnologia', bold: true, alignment: 'center', margin: [0, 3, 0, 0], fontSize: 8 }, { text: 'Endereço da unidade administrativa | CEP: 00000-000 | FONE: (00) 0000-0000', alignment: 'center', fontSize: 8 }]
    });
...
app.get('/api/reports/substitution-full/:returnId/:exitId', authenticateToken, async (req, res) => {
  ...
  try {
    // 1. CARREGAR LOGOTIPO
    const logoPath = path.join(__dirname, 'assets/institution-logo.png'); // Ajuste se a pasta for '../assets'
    let logoBase64 = null;
    if (fs.existsSync(logoPath)) logoBase64 = fs.readFileSync(logoPath, 'base64');
...
app.post('/api/reports/legacy-disposal', authenticateToken, async (req, res) => {
    try {
        ...
        let logoBase64 = null;
        const pathsToTry = [ path.join(__dirname, 'assets/institution-logo.png'), path.join(__dirname, '../assets/institution-logo.png'), path.join(__dirname, 'public/assets/institution-logo.png') ];
